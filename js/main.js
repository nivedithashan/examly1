...
function loadDDL() {
    ...
    var settings = {
        ...
        "headers": {
            ...
            "x-rapidapi-key": "2d57b10c1cmshd2b7500d7d8ccb4p14099bjsnc7cab57b8eb8"
        }
    }
    ...
    $.ajax(settings).done(function (response) {
        ...
        $.each(response, function (k, v) {
            $('#select-language').append('<option value="' + v.id + '">' + v.name + '</option>');
        });
    });
}
...
function run() {
    ...
    var sendRequest = function (data) {
        ...
        $.ajax(settings).done(function (response) {
            console.log(console.log(`Your submission token is: ${response.token}`));
